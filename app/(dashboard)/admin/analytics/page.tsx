...
import { Eye, MousePointerClick, DollarSign, TrendingUp, Loader2 } from 'lucide-react'
...
export default function AnalyticsPage() {
  ...
  const conversionData = stats.dailyData.map(d => ({
    date: d.date,
    conversion: d.visitors > 0 ? Number(((d.clicks / d.visitors) * 100).toFixed(1)) : 0
  }))
  ...
  return (
    <div>
        ...
        <div className="bg-white rounded-xl p-5 shadow-sm border">
          <div className="flex justify-between items-start">
            <div>
              ...
              <p className="text-2xl font-bold mt-1">Rp{stats.estimatedCommission.toLocaleString('id-ID')}</p>
            </div>
      ...
      {/* Conversion Rate Chart - Menggunakan LineChart saja agar tidak error */}
      <div className="bg-white rounded-xl p-5 shadow-sm border mb-8">
        <h3 className="font-semibold mb-4">Conversion Rate by Day (%)</h3>
        <ResponsiveContainer width="100%" height={300}>
